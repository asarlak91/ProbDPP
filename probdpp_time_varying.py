#!/usr/bin/env python3
"""ProbDPP HotpotQA time-varying reliability severity experiment.

This standalone script combines the time-varying experiment and the true
ProbDPP objective-gap analysis. It runs four controlled reliability shifts:
slight, moderate, large, and extreme. All shifts preserve the same mean
reliability while changing which sources are reliable.

Default design:
  * N=10 candidate passages, K=3 selected.
  * T=600 rounds/questions.
  * reliability switch after round 300 (new regime begins at round 301).
  * 30 random seeds.
  * 10-round blocks for plotting.
  * four shift severities with mean absolute change
        Delta_alpha = (1/N) sum_i |alpha_after_i - alpha_before_i|
    equal to 0.05, 0.15, 0.30, and 0.50.
  * mean reliability is preserved at 0.70 in every regime.

Methods:
  * Fixed ProbDPP: always uses the pre-change alpha vector.
  * Online ProbDPP: all-history KL-UCB.
  * Adaptive ProbDPP: sliding-window KL-UCB.
  * Oracle ProbDPP: uses the true current alpha vector.

For every saved selection, the script also computes the TRUE ProbDPP objective
gap using both diversity and reliability:

    gap_t(m) = max_{|S|=K} f_{alpha_t}(S) - f_{alpha_t}(S_t^m),

where

    f_alpha(S) = log det(G[S,S]) + sum_{i in S} r(alpha_i, epsilon).

The Oracle gap is exactly zero up to numerical precision.

Scope note: the sliding-window Adaptive ProbDPP experiment is an empirical
nonstationary extension. It is not covered by the stationary KL-UCB regret
theorem and should not be interpreted as a dynamic-regret guarantee.
"""
